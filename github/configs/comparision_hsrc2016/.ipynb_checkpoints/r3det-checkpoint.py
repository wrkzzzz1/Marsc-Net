...
train_pipeline = [
    ...
    dict(type='RResize', img_scale=[(800,800), (1024,1024), (1280,1280)], keep_ratio=True),
    dict(type='RRandomFlip', flip_ratio=[0.25, 0.25, 0.25], direction=['horizontal', 'vertical', 'diagonal'], version=angle_version),
    dict(type='PolyRandomRotate', rotate_ratio=0.5, mode='value', angles_range=[0, 90, 180, 270], auto_bound=False, version=angle_version),
...
data = dict(
    ...
    val=dict(
        ...
        classwise=True,
        version='le90'))
train_loop = dict(type='EpochBasedTrainLoop', max_epochs=48, val_interval=6)
    ...
    train_cfg=dict(
        ...
        sr=[
            dict(
                assigner=dict(
                    type='MaxIoUAssigner',
                    pos_iou_thr=0.6, neg_iou_thr=0.5,
                    min_pos_iou=0, ignore_iof_thr=-1,
                    iou_calculator=dict(type='RBboxOverlaps2D')
                ),
                allowed_border=-1, pos_weight=-1, debug=False
            ...
            dict(
                assigner=dict(
                    type='MaxIoUAssigner',
                    pos_iou_thr=0.7, neg_iou_thr=0.6,
                    min_pos_iou=0, ignore_iof_thr=-1,
                    iou_calculator=dict(type='RBboxOverlaps2D')
                ),
                allowed_border=-1, pos_weight=-1, debug=False
...
work_dir = '/root/mmdetection/work_dirs/r3det-36turns'
auto_resume = False
gpu_ids = range(0, 1)
seed = 4
deterministic = True
