        ...
        for key in results.get('bbox_fields', []):
            ...
            if isinstance(scale_factor, (list, tuple, np.ndarray)):
                if len(scale_factor) == 4:
                    ...
                    raise ValueError(f'Unexpected scale_factor shape: {scale_factor}')
            else:
                raise TypeError(f'Unexpected type for scale_factor: {type(scale_factor)}')
            if bboxes.shape[0] == 0:
            # 空bbox时直接跳过resize，防止广播错误
                results[key] = bboxes.reshape(orig_shape)
                continue
            
            bboxes[:, 0] = bboxes[:, 0] * w_scale
            bboxes[:, 1] = bboxes[:, 1] * h_scale
            bboxes[:, 2:4] = bboxes[:, 2:4] * np.sqrt(w_scale * h_scale)
