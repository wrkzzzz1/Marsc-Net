...
class RotatedAnchorHead(BaseDenseHead):
    ...
    def get_targets(self,
        ...
        # ✅ 修复点：对 rest_results 自动对齐维度
        aligned_rest = []
        for r in rest_results:
            # r 是 list[Tensor], 每张图一个
            max_dim = max([t.size(-1) for t in r])  # 找到这一批次最大维度
            padded_r = []
            for t in r:
                if t.size(-1) < max_dim:
                    # 用零填充到相同维度
                    pad_size = (0, max_dim - t.size(-1))
                    t = torch.nn.functional.pad(t, pad_size, value=0)
                padded_r.append(t)
            # 现在 batch 内 shape 一致，可以安全 images_to_levels
            aligned_rest.append(images_to_levels(padded_r, num_level_anchors))

        return res + tuple(aligned_rest)
